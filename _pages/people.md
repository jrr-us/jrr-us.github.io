---
layout: default
title: people
permalink: /people/
nav: true
nav_order: 2
---

<div class="row row-cols-1 row-cols-md-4 px-4 px-md-0">
    <!-- JRR -->
    <div class="col mb-4">
        <div class="card hoverable h-100">
            {% include figure.liquid path="assets/img/people/avatar-jrr.jpg" title="J. Ramirez-Rico" class="card-img-top" %}
            <div class="card-body py-3 d-flex flex-column">
                <h5 class="card-title mb-1">J. Ramirez-Rico</h5>
                <p class="card-text mb-1">Full Professor - US</p>
                <a href="/about-jrr/" class="mt-auto">Go to full bio 📖</a>
            </div>
        </div>
    </div>
    <!-- MDA -->
    <div class="col mb-4">
        <div class="card hoverable h-100">
            {% include figure.liquid path="assets/img/people/avatar-mda.jpg" title="María D. Alba" class="card-img-top" %}
            <div class="card-body py-3 d-flex flex-column">
                <h5 class="card-title mb-1">María D. Alba</h5>
                <p class="card-text mb-1">Research Scientist - CSIC</p>
                <a href="/about-mda/" class="mt-auto">Go to full bio 📖</a>
            </div>
        </div>
    </div>
    <!-- EP -->
    <div class="col mb-4">
        <div class="card hoverable h-100">
            {% include figure.liquid path="assets/img/people/avatar-epg.jpg" title="One" class="card-img-top" %}
            <div class="card-body py-3 d-flex flex-column">
                <h5 class="card-title mb-1">Esperanza Pavón</h5>
                <p class="card-text mb-1">Associate Professor - US</p>
                <a href="/about-epg/" class="mt-auto">Go to full bio 📖</a>
            </div>
        </div>
    </div>
    <!-- JC -->
    <div class="col mb-4">
        <div class="card hoverable h-100">
            {% include figure.liquid path="assets/img/people/avatar-jc.jpg" title="One" class="card-img-top" %}
            <div class="card-body py-3 d-flex flex-column">
                <h5 class="card-title mb-1">João Coelho</h5>
                <p class="card-text mb-1">EMERGIA Fellow - US</p>
                <a href="/about-jc/" class="mt-auto">Go to full bio 📖</a>
            </div>
        </div>
    </div>
    <!-- MZ -->
    <div class="col mb-4">
        <div class="card hoverable h-100">
            {% include figure.liquid path="assets/img/people/avatar-mz.jpg" title="One" class="card-img-top" %}
            <div class="card-body py-3 d-flex flex-column">
                <h5 class="card-title mb-1">Maider Zarrabeitia</h5>
                <p class="card-text mb-1">Tenured Scientist - CSIC</p>
                <a href="/about-mz/" class="mt-auto">Go to full bio 📖</a>
            </div>
        </div>
    </div>
    <!-- ARAL -->
    <div class="col mb-4">
        <div class="card hoverable h-100">
            {% include figure.liquid path="assets/img/people/avatar-aral.jpg" title="One" class="card-img-top" %}
            <div class="card-body py-3 d-flex flex-column">
                <h5 class="card-title mb-1">A. R. de Arellano</h5>
                <p class="card-text mb-1">Full Professor - US</p>
                <a href="/about-aral/" class="mt-auto">Go to full bio 📖</a>
            </div>
        </div>
    </div>
    <div class="col mb-4">
        <div class="card hoverable h-100">
            {% include figure.liquid path="assets/img/people/avatar-jjb.jpg" title="One" class="card-img-top" %}
            <div class="card-body py-3 d-flex flex-column">
                <h5 class="card-title mb-1">Jesús Benítez</h5>
                <p class="card-text mb-1">Tenured Scientist - CSIC</p>
                <a href="/about-jjb/" class="mt-auto">Go to full bio 📖</a>
            </div>
        </div>
    </div>
    <div class="col mb-4">
        <div class="card hoverable h-100">
            {% include figure.liquid path="assets/img/people/avatar-agm.jpg" title="One" class="card-img-top" %}
            <div class="card-body py-3 d-flex flex-column">
                <h5 class="card-title mb-1">Aurora Gómez</h5>
                <p class="card-text mb-1">Postdoctoral Researcher</p>
                <a href="/about-agm/" class="mt-auto">Go to full bio 📖</a>
            </div>
        </div>
    </div>
</div>

